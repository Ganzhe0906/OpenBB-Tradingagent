...
Data provider interfaces for economic analysis.

Implemented data sources:
  - FRED (Federal Reserve Economic Data) — US macro series
  - ManualDataInput — user-provided datasets and CSV imports

FRED API key is configured via tools.financial.fredApiKey in
~/.finclaw/config.json or the FRED_API_KEY environment variable.
"""

import json
import httpx
import httpx as requests  # sync-compat alias
import pandas as pd
import numpy as np
...
import logging
from ..base.base_models import EconomicsBase, ValidationError, CalculationError, DataError, DataContainer

logger = logging.getLogger(__name__)


def get_fred_api_key() -> str:
    """Resolve FRED API key from ~/.finclaw/config.json or env var.

    Lookup order:
      1. ``tools.financial.fredApiKey`` in config.json
