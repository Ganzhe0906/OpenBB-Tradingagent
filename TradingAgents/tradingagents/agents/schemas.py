...
class PortfolioDecision(BaseModel):
    ...
    novice_guide: Optional[str] = Field(
        default=None,
        description=(
            "新手与小白投资指南 (用中文书写)。这是一个面向非专业小白投资者的简明指引，"
            "必须包含以下三个子部分，且各部分之间用换行隔开：\n"
            "- 【整体方向】说明目前是看空、看多还是观望，以及评级的白话解释；\n"
            "- 【仓位建议】清晰给出三类用户的应对方案：\n"
            "  1. 满仓/重仓用户的具体操作建议\n"
            "  2. 半仓/轻仓用户的具体操作建议\n"
            "  3. 空仓/不持仓用户的具体操作建议\n"
            "- 【辩论盘点】盘点分析师团队在本轮辩论中的多空焦点、各方论据的优劣势与合理性评估（例如谁的论点更有力、谁的论点过于激进或保守，以大白话解析他们的博弈精髓，字数约200-300字）。"
        ),
    )
...
def render_pm_decision(decision: PortfolioDecision) -> str:
    ...
    """
    parts = []
    if hasattr(decision, "novice_guide") and decision.novice_guide:
        parts.extend([
            "### 🔰 新手与小白投资指南 (Beginner's Guide)",
            decision.novice_guide,
            "",
            "---",
            ""
        ])
